...
<div className='new-login-background new-admin-login'>
  ...
  <form>
    ...
    <button className=" buttonw new-alert-close-button" onClick={() => navigate('/')}>
         Go Back
    </button>
    ...
    {showAlert && (
      <div className="new-alert-container new-admin-alert">
            <p class='para'>Unauthorized! Find the secret handshake or face the password fairy's magic! 🚪✨</p>

        <p className="new-alert-message">{alertMessage}</p>
        <button className="new-alert-close-button" onClick={() => setShowAlert(false)}>
          Close
        </button>
      </div>
    )}
